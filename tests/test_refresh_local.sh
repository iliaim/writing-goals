...
mkdir -p "$status_root/scripts" "$TEST_TMP/claude-home/.claude/skills" "$TEST_TMP/claude-home/.codex/skills/writing-goals" "$TEST_TMP/codex-home/skills"
cp "$refresh" "$status_root/scripts/refresh-local.sh"
mkdir -p "$TEST_TMP/claude-source"
ln -s "$TEST_TMP/claude-source" "$TEST_TMP/claude-home/.claude/skills/writing-goals"

run_command env -u CODEX_HOME HOME="$TEST_TMP/claude-home" bash "$status_root/scripts/refresh-local.sh" --status
...
TEST_COUNT=$((TEST_COUNT + 1))
if [ -d "$TEST_TMP/claude-home/.codex/skills/writing-goals" ]; then
  pass 'G09_SAFE_REFRESH: status uses and preserves the default Codex target'
else
  fail 'G09_SAFE_REFRESH: status uses and preserves the default Codex target'
fi
...
__END__" ]; then
  pass 'G09_SAFE_REFRESH: status uses the Codex override and reports the newest absolute archive exactly'
else
  fail "G09_SAFE_REFRESH: status uses the Codex override and reports the newest absolute archive exactly (got $(tr '\n' '|' < "$RUN_OUT"))"
fi
