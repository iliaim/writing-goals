...
Three ways to run a goal (or a decomposed chain). They differ only in **who
...
does not update the next goal record automatically.
...
- reads the current goal record and its **gate output**, and
...
| Any irreversible / external / spend step in the path | **1**, or pause mode 3 for human confirmation that names the action and bounds |
...
- **Entry re-verify:** the driver re-checks the previous goal's gate at entry — it never
  trusts a stored completion marker.
