...
immutable and separate from the mutable plan. Each goal record names a stable id, one-line title,
...
The workspace records let a new session recover the plan without relying on in-memory context.
