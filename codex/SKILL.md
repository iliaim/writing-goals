...
- **`PreToolUse`** on Codex intercepts **shell only** (not Edit/Write) → **pair the deny-list with workspace/dir scoping** so file-level dangers are covered. Its stdin is Claude-identical (`tool_name:"Bash"`, `tool_input.command`, `cwd`), so the shared **`assets/deny-list.sh`** works verbatim (it resolves the repo root via its `$PWD` fallback).
...
- `shared/gates.md` — deterministic `Stop`-hook scaffold + iteration counter (see Codex block contract)
